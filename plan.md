...
- **Paso activo:** **FASE 5 COMPLETA** (Pasos 1-6) + **paso de seguridad hecho** (MFA en admin + throttling en ambos logins). Fase 4 cerrada v1 (6/7/9/10 hechos, 8 base, 11→6.5/v2.0.0). Siguiente: **Fase 6 — agente IA**, y después la **Fase 6.5** nueva.
...
- **Rama activa:** `feat/seguridad-mfa-throttling` (MFA + throttling, `verify` verde, **sin commitear** — pendiente del visto bueno). Al mergear, esta línea vuelve a `main`. **Fase 5 Paso 6 — y con él la FASE 5 ENTERA — entró vía PR #19** (`feat/f5p6-cta-leads`, squash, CI verde, rama borrada). Antes: revisión de seguridad **PR #18**, Paso 5 **PR #17**, Paso 4 **PR #14**. Antes, `feat/6a-builder-editor` se integró entera vía **PR #12** (squash, 11 commits, 69 archivos, CI verde) y se borró local y remoto. `main` acumula: Paso 4 (PR #10), Paso 5 (PR #11) y Fase 4 completa + Fase 5 P1-3 (PR #12). Se vuelve al flujo normal de **una rama por paso**.
