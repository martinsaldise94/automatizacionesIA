...
- **Paso activo:** **FASE 5 — Paso 4 (blog público) CERRADO.** Fase 4 cerrada v1 (6/7/9/10 hechos, 8 base, 11→v2.0.0). Fase 5 Pasos 1+2+3+4 hechos.
...
- **Siguiente:** Paso 5 (editor de blog en `(portal)/blog`: crear/editar/publicar con markdown + preview — `lib/db/posts.ts` ya tiene `listPostsForTenant`/`getPostForEditor`/`createPost`/`updatePost`/`deletePost`; reusar `<Markdown>` para el preview y el patrón de actions+guard de `(portal)/builder`). Paso 6 (CTA derivación WhatsApp/formulario — el header público ya tiene botón "Contactar" wa.me; falta el CTA/flujo de formulario).
...
- **Tests:** **210 verde** en 27 archivos (`npm run verify` completo: tsc + eslint + vitest). `npm run build` también verde.
...
- **Rama activa:** `main`, limpia y al día (esta línea se escribe SIEMPRE en estado post-merge: al cerrar un paso la rama ya está mergeada y borrada). Fase 5 Paso 4 entró vía **PR #14**. Antes, `feat/6a-builder-editor` se integró entera vía **PR #12** (squash, 11 commits, 69 archivos, CI verde) y se borró local y remoto. `main` acumula: Paso 4 (PR #10), Paso 5 (PR #11) y Fase 4 completa + Fase 5 P1-3 (PR #12). Se vuelve al flujo normal de **una rama por paso**.
- **Entorno dev:** dev server en **:3001** (el 3000 lo ocupa una app Express externa). Contraseña dev del dueño casa-frodo: `msaldise@zaldicode.com` / `BuilderTest123!`.
