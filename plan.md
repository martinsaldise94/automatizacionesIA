...
- **Paso activo:** **FASE 5 COMPLETA** (Pasos 1-6). Fase 4 cerrada v1 (6/7/9/10 hechos, 8 base, 11→v2.0.0). Siguiente: paso de seguridad (MFA admin + throttling dueños) y luego **Fase 6 — agente IA**.
...
- **Siguiente:** paso de seguridad (MFA en admin + throttling en el login de dueños; ver `references/security.md` — comprobar antes qué límites trae ya Supabase Auth). Luego **Fase 6 — agente IA**, que necesita `ANTHROPIC_API_KEY` en `.env.local`. Decisión de arquitectura ya tomada e investigada: agente propio sobre Claude Haiku, no chatbot de terceros (Botpress queda como plan B documentado).
...
- **Tests:** **277 verde** en 31 archivos (`npm run verify` completo: tsc + eslint + vitest). `npm run build` también verde.
...
- **Rama activa:** `main`, limpia y al día (esta línea se escribe SIEMPRE en estado post-merge: al cerrar un paso la rama ya está mergeada y borrada). **Fase 5 Paso 6 — y con él la FASE 5 ENTERA — entró vía PR #19** (`feat/f5p6-cta-leads`, squash, CI verde, rama borrada). Antes: revisión de seguridad **PR #18**, Paso 5 **PR #17**, Paso 4 **PR #14**. Antes, `feat/6a-builder-editor` se integró entera vía **PR #12** (squash, 11 commits, 69 archivos, CI verde) y se borró local y remoto. `main` acumula: Paso 4 (PR #10), Paso 5 (PR #11) y Fase 4 completa + Fase 5 P1-3 (PR #12). Se vuelve al flujo normal de **una rama por paso**.
