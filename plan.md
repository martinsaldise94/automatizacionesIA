...
- **Paso activo:** **FASE 5 — Paso 5 (editor de blog) CERRADO.** Fase 4 cerrada v1 (6/7/9/10 hechos, 8 base, 11→v2.0.0). Fase 5 Pasos 1+2+3+4+5 hechos.
...
- **Siguiente:** Paso 6, último de la Fase 5 (CTA de derivación a WhatsApp/formulario visible en la web — el header público ya tiene botón "Contactar" wa.me; falta el CTA en la página y el flujo de formulario → `leads`). Con eso cierra la Fase 5 y entra la Fase 6 (agente IA), que necesita `ANTHROPIC_API_KEY` en `.env.local`.
...
- **Tests:** **240 verde** en 28 archivos (`npm run verify` completo: tsc + eslint + vitest). `npm run build` también verde.
