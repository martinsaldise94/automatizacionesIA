...
- [~] Paso 3 — SEO. **WIP SIN COMMITEAR NI VERIFICAR** (ver bloque `Contexto actual`). generateMetadata + JSON-LD LocalBusiness + sitemap.xml/robots.txt por tenant (route handlers + proxy). Falta: `npm run verify`, test del rewrite de sitemap en middleware, browser-test, commit.
...
- **`npm run verify` verde:** 174 tests en 26 archivos (incluye ya `seo` y `sitemap` del WIP). Corregido: la línea de abajo decía "sin correr aún".
- **Paso activo:** **FASE 5 — Paso 3 (SEO) EN CURSO, SIN COMMITEAR** — verify pasa, pero **falta** el caso de test del rewrite en `tests/middleware.test.ts` y la verificación en navegador (sesión cortada aquí). Fase 4 cerrada v1 (6a/6b/7/9/10 hechos, 8 base, 11→v2.0.0). Fase 5 Pasos 1+2 HECHOS y committeados (commit 8).
- **⚠️ WIP sin commitear (Paso 3 SEO) — AL RETOMAR: correr `npm run verify` primero, arreglar lo que falle, browser-test, y commitear.** Archivos tocados/nuevos sin commit:
  - `lib/seo.ts` (+`tests/seo.test.ts`): `pageTitle`, `pageMetadata`, `localBusinessJsonLd` (puras).
  - `lib/sitemap.ts` (+`tests/sitemap.test.ts`): `absUrl`, `buildSitemapXml`, `buildRobotsTxt`, `originFromHeaders`.
  - `lib/db/posts.ts` (NUEVO, capa de datos del blog: lecturas públicas `listPublishedPosts`/`listPublishedPostSlugs`/`getPublishedPost` + editor `listPostsForTenant`/`getPostForEditor`/`createPost`/`updatePost`/`deletePost`). Ya sirve para Pasos 4 y 5.
  - `app/[tenant]/(public)/[[...path]]/page.tsx`: añadido `generateMetadata` (usa `pageMetadata`).
  - `app/[tenant]/(public)/layout.tsx`: añadido `<script ld+json>` con `localBusinessJsonLd`.
  - `proxy.ts`: excluye `/sitemap.xml` y `/robots.txt` del skip por extensión (para reescribirlos por tenant). **PENDIENTE: añadir caso de test en `tests/middleware.test.ts` para ese rewrite.**
  - `app/[tenant]/(public)/sitemap.xml/route.ts` y `.../robots.txt/route.ts` (route handlers por tenant).
  - **Verificar en navegador (demo-clinica :3001... OJO el dev quedó en :3001, el 3000 lo ocupa una app Express):** `<title>`/meta, JSON-LD en el head, `/sitemap.xml` y `/robots.txt` devuelven XML/txt del tenant. **Riesgo a comprobar:** que las route handlers `sitemap.xml`/`robots.txt` no choquen con el catch-all `[[...path]]` (deberían ganar por ser concretas).
- **Siguiente tras cerrar Paso 3:** Paso 4 (blog público: `(public)/blog` lista + `blog/[slug]` con markdown seguro — reusar `react-markdown` como en RichText; `lib/db/posts.ts` ya listo). Paso 5 (editor blog en `(portal)/blog`). Paso 6 (CTA derivación WhatsApp/formulario — el header público ya tiene botón "Contactar" wa.me; falta el CTA/flujo de formulario).
...
- **Tests:** 164 verde en el último commit (commit 8). El WIP de Paso 3 añade `seo`+`sitemap` (sin correr aún). tsc+eslint+build de prod limpios en commit 8.
- **Rama:** `feat/6a-builder-editor`, 8 commits sin pushear/PR (por decisión del usuario, todo en esta rama). Dev server quedó en **:3001** (3000 ocupado por app Express externa). Contraseña dev del dueño casa-frodo: `msaldise@zaldicode.com` / `BuilderTest123!`.
