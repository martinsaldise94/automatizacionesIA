...
- [x] Paso 8 — HECHO (`verify` verde, 373 tests). **Campo de imagen cableado en Puck**: los 6 campos de imagen (`Hero.image`, `TextImage.image`, `Gallery.items[].image`, `Team.items[].photo`, `LogoGrid.items[].logo`, `Testimonials.items[].avatar`) pasan de `type:'text'` —que obligaba a **pegar una URL**, algo que un dueño no técnico no tiene por ningún lado— a `imageField()` (`type:'custom'`), que reutiliza el `ImagePicker` del blog: subir del equipo o pegar URL, con miniatura y "Quitar". **Cargado en DIFERIDO (`next/dynamic`, `ssr:false`)** y esa es la decisión que importa: `PublicRender` es cliente e importa `builderConfig` ENTERA, así que sin diferir, el selector y la server action de subida viajarían al navegador de cada visitante de la web pública — peso que pagaría alguien que solo entró a mirar horarios. Archivos: `components/builder/imageField.tsx` (**SIN `'use client'`**) + `components/builder/LazyImagePicker.tsx` (`'use client'`, aquí vive el `dynamic`) + `components/builder/PuckImagePicker.tsx` (adaptador que ata la action fuera de la config) +`tests/imageFields.test.ts` (5, estructural: recorre TODOS los campos incluidos los de dentro de arrays y falla si un campo de imagen nuevo se queda en `type:'text'` — es fácil olvidarlo porque compila igual; `imageAlt` queda excluido a propósito, es SEO y no una foto). **⚠️ Sigue pendiente aplicar `0007_storage.sql`**: sin el bucket, la subida devuelve un error explícito que lo dice y la vía URL sigue funcionando. Detalle previo (base): `supabase/migrations/0007_storage.sql`. `supabase/migrations/0007_storage.sql`: bucket `tenant-assets` (público lectura, 5MB, jpg/png/webp) + policy de lectura; escrituras solo service role. `lib/builder/upload.ts`: `validateImageFile` (MIME real, no extensión; **SVG prohibido**), `extForImageMime`, `buildAssetPath` (`{tenant_id}/{uuid}.{ext}`) +`tests/upload.test.ts` (11). `uploadImageAction` **movida a `(portal)/upload.ts`** (archivo propio: la comparten blog y el futuro campo de Puck, y así se evita el ciclo actions↔config que estaba anotado como pendiente). `components/portal/ImagePicker.tsx` (NUEVO): campo reutilizable **URL o archivo del equipo**, con miniatura, "Quitar" y validación en cliente + servidor (el mismo texto vía `imageErrorMessage`). **Ya enganchado a la portada del blog (Paso 5).** **⚠️ QUEDA:** (1) aplicar `0007_storage.sql` en Supabase para crear el bucket — sin eso la subida devuelve un error que lo dice explícitamente; (2) usar `ImagePicker` como campo `type:'custom'` en los bloques de Puck, reemplazando los `type:'text'` de imagen; (3) smoke de subida real. En bloques, hoy las imágenes siguen funcionando pegando URL.
...
### Ya anticipado (fuera de fase, a petición del usuario)

Se apunta aquí para que el alcance de la 6.5 refleje lo que ya existe y no se replanifique dos veces.

- **[HECHO] Imagen de fondo de sección con overlay** — era el ítem (c) heredado del roadmap v2. Implementado como cuarta variante del Hero (`image-background`) con control de oscurecido de tres niveles. Ver `## Contexto actual`.

**Dónde está la línea para lo que queda.** Añadir **variantes** a bloques existentes se puede seguir haciendo suelto: no cambia cómo se usa el editor, solo lo que se consigue con él. Lo que **no** se toca fuera de la fase es **multicolumna, espaciado y ajustes por sección**: eso cambia el modelo de interacción (arrastre, selección, árbol de capas, panel lateral funcionan distinto con zonas anidadas), y meterlo a trozos garantiza tener que repetir la pasada de UX sobre una planta que va a cambiar. Es el mismo argumento por el que UX y potencia van juntas en una sola fase.

### Fricciones observadas (ir rellenando)

- **[RESUELTA] El Hero descartaba la imagen sin decir nada.** La variante por defecto es "solo texto, centrado", que ignora `image` a propósito. Pero el campo de imagen se mostraba igual: el usuario subió una foto, se guardó, y el bloque la tiró. Desde fuera eso no parece una opción de diseño — parece que la plataforma está rota. Arreglado con `resolveFields` (el campo solo aparece si el diseño usa imagen) y renombrando las opciones para que digan cuál lleva foto. **Regla que sale de aquí: no se ofrece un control que no hace nada.** Vale para todo el builder, no solo el Hero.
- *(seguir anotando cada atasco, una línea por atasco)*
...
- **Rama activa:** `main`, limpia y al día. Últimos merges: **PR #22** (agente: prompt, conversación, derivación, blindaje anti-inyección), **PR #23** (campo de imagen del builder + Hero con imagen de fondo), **PR #24** (MIME por magic bytes, matcher del proxy, least privilege en posts). **393 tests** verde.
- **[Fuera de fase] Tres puntos abiertos de `references/security.md`, cerrados.** (1) **MIME por magic bytes**: `file.type` lo declara el navegador, así que renombrar un `.html` a `.jpg` pasaba la validación entera. `sniffImageMime` mira la cabecera real y **a partir de ahí se ignora lo que dijo el cliente** — el formato detectado se usa también para el `contentType` y la extensión, así que un desajuste no se cuela. En WEBP hay que comprobar los DOS trozos (`RIFF`…`WEBP`): un WAV también empieza por `RIFF`. +`tests/imageSniff.test.ts` (7). (2) **Matcher del proxy**: lo excluido del matcher NO pasa por el proxy, y el proxy es quien borra el `x-tenant` del cliente; excluir por extensión parecía optimización y era agujero latente (el día que exista un `opengraph-image` o un `route.ts` que devuelva PNG). Ahora solo se excluyen `_next/static` y `_next/image`. **Regla: si una ruta puede alcanzar código nuestro, pasa por el proxy.** (3) **`posts_public_read` retirada** (`0011`): el blog va por service role (verificado, `lib/db/posts.ts` es el único sitio que toca la tabla), `anon` no necesitaba nada. No arregla una fuga —los posts no tienen borrador— sino que quita los grants por defecto que serían catastróficos sin RLS. **⚠️ Pendiente aplicar `0011_posts_least_privilege.sql`.**
- **Hero: cuarta variante `image-background`** (foto detrás del texto, a petición del usuario). Lo que decide si se ve profesional o cutre es el **contraste del texto sobre la foto**, así que el velo oscuro (`overlay`: Poco/Medio/Fuerte, por defecto Medio) es parte del bloque, no un extra. **No se ofrece "Ninguno"**: sobre una foto clara dejaría el titular invisible y el dueño no tendría forma de saber que el problema es su foto — hay test que lo fija. `Section background="dark"` debajo no es redundante: si la foto no carga, queda fondo oscuro y el texto blanco se sigue leyendo. `isolate` acota el `-z-10` (sin él la foto se colaría por detrás de la página entera). Sin foto, degrada al layout centrado en vez de pintar una sección oscura vacía. El selector de fondo (blanco/primario/gris) **desaparece** en esta variante: no se ve, sería otro control que no hace nada. Los tres niveles están en `/dev/preview` para juzgar el contraste sobre una foto real. **El bucket de Storage está aplicado y la subida funciona** (verificado: `uploadImageAction` devolvió URL sin error).
- **Primera fricción real del builder, encontrada probando y ya en la lista de la Fase 6.5:** el Hero mostraba el campo de imagen incluso en la variante "centrado", que la descarta a propósito → el dueño sube una foto y no pasa nada. **Regla que sale de ahí y aplica a todo el builder: no se ofrece un control que no hace nada.** Resuelto con `resolveFields` de Puck (`lib/builder/heroFields.ts`, puro y testeado) + opciones renombradas a "Solo texto, centrado" / "Texto + imagen a la izquierda|derecha".
- **Gotcha — `lib/builder/config.tsx` se evalúa TAMBIÉN en el servidor.** Lo importan las plantillas de alta, la validación al publicar y el sitemap, no solo el editor. Por eso **nada de lo que la config *llama* puede vivir en un módulo `'use client'`**: desde el servidor una función exportada así no es una función, es una referencia, y el módulo revienta al evaluarse (*"Attempted to call imageField() from the server"*). Un componente de cliente sí puede **renderizarse** desde ahí; lo que no puede es invocarse. Patrón que resuelve ambas cosas: la fábrica del campo en un módulo neutro (`imageField.tsx`) y el trozo que necesita hooks de cliente —`dynamic({ssr:false})`— en otro con `'use client'` (`LazyImagePicker.tsx`). **Los tests no lo cazaron: en vitest la directiva es inerte** y el módulo importa igual, así que hay un test de código fuente que fija la frontera entre los dos archivos. La tanda de Fase 6 (pasos 1, 3, 4 y blindaje del 6) entró en `main` vía **PR #22**.
- **Estrategia mientras no haya `ANTHROPIC_API_KEY`: cerrar frentes abiertos, no abrir la Fase 7.** Razonado con el usuario: la Fase 7 es tier_2 y el tier_1 aún no se puede vender sin el agente, así que construirla dejaría dos productos incompletos en vez de uno terminado; además su Paso 2 (Cal.com vs motor propio) es una decisión cara de revertir que conviene tomar con clientes reales delante, no a ciegas. Siguiente frente a cerrar: **puntos abiertos de `references/security.md`** (MIME por magic bytes, huecos del matcher del proxy, `posts_public_read`). **Decisión del usuario: seguir sin la API de Anthropic hasta que sea imprescindible** → se hacen antes los pasos que no la necesitan (1 prompt, 3 persistencia, y a continuación 4 handoff), y el Paso 2 (endpoint) + 5 (widget) quedan para cuando exista la clave. **⚠️ Pendiente aplicar `0010_messages_conversation.sql`.** El paso de seguridad entró en `main` vía **PR #21**. **Fase 5 Paso 6 — y con él la FASE 5 ENTERA — entró vía PR #19** (`feat/f5p6-cta-leads`, squash, CI verde, rama borrada). Antes: revisión de seguridad **PR #18**, Paso 5 **PR #17**, Paso 4 **PR #14**. Antes, `feat/6a-builder-editor` se integró entera vía **PR #12** (squash, 11 commits, 69 archivos, CI verde) y se borró local y remoto. `main` acumula: Paso 4 (PR #10), Paso 5 (PR #11) y Fase 4 completa + Fase 5 P1-3 (PR #12). Se vuelve al flujo normal de **una rama por paso**.
